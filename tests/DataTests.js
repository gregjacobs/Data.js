	...
	items: [
		{
			...
			setUp : function() {
				...
				this.Attribute = Data.attribute.Attribute.extend( {} );
			},
		 ...
		 */
		{
			...
			"doSet() should call the Attribute's prototype set() method if there is no 'set' config, with the appropriate arguments" : function() {
				...
				var Attribute = Data.attribute.Attribute.extend( {
...
tests.unit.attribute.add( new Ext.test.TestSuite( {
	...
	items : [
		 ...
		 */
		{
			...
			setUp : function() {
				this.Model = Data.Model.extend( {
					attributes : [ 'attr1', 'attr2' ]
				} );
				
				this.Collection = Data.Collection.extend( {
					model : this.Model
				} );
			...
			"beforeSet() should convert an array of data objects, when collectionClass is a string" : function() {
				...
				window.__Data_CollectionAttributeTest.ns1.ns2.MyCollection = Data.Collection.extend( {
					model : this.Model
				} );
			...
			"beforeSet() should convert an array of data objects, when collectionClass is a function" : function() {
				var TestCollection = Data.Collection.extend( {
					model : this.Model
				} );
	...
	items : [
		 ...
		 */
		{
			...
			"beforeSet() should convert an anonymous data object to the provided modelClass, when modelClass is a string" : function() {
				...
				window.__Data_CollectionAttributeTest.ns1 = {};
				window.__Data_CollectionAttributeTest.ns1.ns2 = {};
				window.__Data_CollectionAttributeTest.ns1.ns2.MyModel = Data.Model.extend( {
					attributes : [ 'attr1', 'attr2' ]
				} );
			...
			"beforeSet() should convert an anonymous data object to the provided modelClass, when modelClass is a function" : function() {
				var TestModel = Data.Model.extend( {
					attributes : [ 'attr1', 'attr2' ]
				} );
...
/*global window, Ext, Y, JsMockito, tests, jQuery, Data */
tests.unit.add( new Ext.test.TestSuite( {
	...
	items : [
		...
		
		{
			...
			"remove() should remove the model from the modelsById hashmap, so it is no longer retrievable by getById" : function() {
				...
				var collection = new Data.Collection( [ model ] );
			...
			"remove() should remove the model from the modelsByClientId hashmap, so it is no longer retrievable by getById" : function() {
				var Model = Data.Model.extend( {} ),
				    ...
				    collection = new Data.Collection( [ model ] );
		...
		
		{
			...
			"getModels() should return the array of models, but in a new array so that the array can be changed" : function() {
				var Model = Data.Model.extend( { attributes: [ 'attr' ] } ),
				    model1 = new Model( { attr: 1 } ),
				    model2 = new Model( { attr: 2 } ),
				    model3 = new Model( { attr: 3 } ),
				    collection = new Data.Collection( [ model1, model2, model3 ] );
		...
		
		{
			...
			"getCount() should return the number of models inserted at any given time" : function() {
				var Model = Data.Model.extend( { attributes: [ 'attr' ] } ),
				    ...
				    model2 = new Model( { attr: 2 } ),
				    model3 = new Model( { attr: 3 } ),
				    collection = new Data.Collection( [ model1, model2 ] );
		...
		
		{
			...
			"getByClientId() should retrieve a model by its clientId" : function() {
				...
				var collection = new Data.Collection( [ model1, model2 ] );
			...
			"getByClientId() should return null if the collection doesn't have the model whose clientId is requested" : function() {
				...
				var collection = new Data.Collection();  // note: not adding model
		...
		
		{
			...
			setUp : function() {
				this.Model = Data.Model.extend( { attributes: [ 'id' ], idAttribute: 'id' } );
			},
			...
			"getById() should retreive a model by its id attribute, even if it doesn't yet have an id when it is added to the collection (the id is added later)" : function() {
				var model = new this.Model(),  // no id yet
				    collection = new Data.Collection( [ model ] );  // add the model
		...
		
		{
			...
			"indexOf() should return -1 for a model that does not exist within the collection" : function() {
				var Model = Data.Model.extend( { attributes: [ 'attr' ] } ),
				    model1 = new Model(),
				    model2 = new Model(),
				    collection = new Data.Collection( [ model1 ] );  // not adding model2
		...
		
		{
			...
			"indexOfId() should return the index of a model by its id in the collection" : function() {
				var Model = Data.Model.extend( { attributes: [ 'id' ], idAttribute: 'id' } ),
		...
		
		{
			...
			setUp : function() {
				this.unmodifiedModel1 = JsMockito.mock( Data.Model );
				...
				this.unmodifiedModel2 = JsMockito.mock( Data.Model );
		 ...
		 */
		{
			 ...
			 * @return {Data.Model[]} The array of mock models.
			 */
			createModels : function( howMany ) {
				var models = [],
				    ConcreteAttribute = Data.attribute.Attribute.extend( { constructor: function(){} } );
				
				for( var i = 0; i < howMany; i++ ) {
					var model = JsMockito.mock( Data.Model ),
			...
			"sync() should create (save) models that are new" : function() {
				...
				var collection = new Data.Collection( models );
				collection.sync();
...
/*global jQuery, Ext, Y, Data, tests, _ */
tests.unit.data.add( new Ext.test.TestSuite( {
	name : 'Data.data.NativeObjectConverter',
	...
	items : [
		 ...
		 */
		{
			...
			"convert() should return a key for each of the Attributes in the Model, whether or not any data has been set to them" : function() {
				...
				var data = Data.data.NativeObjectConverter.convert( model );
			...
			"when the `raw` option is provided as true, convert() should return the data by running attributes' `raw` functions (not using `get`)" : function() {
				...
				var data = Data.data.NativeObjectConverter.convert( model, { raw: true } );
				Y.Assert.areSame( 'value1', data.attribute1, "attribute1 should be 'value1'" );
			...
			"convert() should only retrieve the data for the persisted attributes (i.e. attributes with persist: true) with the `persistedOnly` option set to true" : function() {
				...
				var persistedData = Data.data.NativeObjectConverter.convert( model, { persistedOnly: true } );
			...
			"convert() should only retrieve the data for the persisted attributes in nested models (i.e. attributes with persist: true) with the `persistedOnly` option set to true" : function() {
				var ParentModel = Data.Model.extend( {
					attributes : [
						{ name: 'id', type: 'string' },
				...
				var persistedData = Data.data.NativeObjectConverter.convert( parentModel, { persistedOnly: true } );
			...
			"convert() should only process the attributes of a Model that are defined by the 'attributeNames' option (if provided)" : function() {
				...
				var data = Data.data.NativeObjectConverter.convert( model, { attributeNames: [ 'attribute1', 'attribute3' ] } );
			...
			"Using the 'attributeNames' option should only affect the Model that is provided to convert(), not nested models" : function() {
				...
				var data = Data.data.NativeObjectConverter.convert( model, { attributeNames: [ 'attribute1' ] } );
			...
			"convert() should deep convert nested models, while handing circular references" : function() {
				...
				var data = Data.data.NativeObjectConverter.convert( outerModel );
		 ...
		 */
		{
			...
			"convert() should convert a Collection of Models into an Array of Objects" : function() {
				...
				var data = Data.data.NativeObjectConverter.convert( collection );
			...
			// This is for the "related" collection hack, when we need to persist it to the server... See note in doc comment about 'raw' option for Data.data.NativeObjectConverter.convert()
			...
			"convert() should simply return an array of objects with only an id property for each of the models in a Collection if 'raw' is true and the collection is not 'embedded'" : function() {
				...
				var data = Data.data.NativeObjectConverter.convert( parentModel, { raw: true } );
...
tests.unit.add( new Ext.test.TestCase( {
	...
	setUp : function() {
		...
		this.MockModel1.__Data_modelTypeId = "1";
		
		this.MockModel2 = function() {};
		this.MockModel2.__Data_modelTypeId = "2";
	...
	"get() should return a reference to the same model provided to it if not providing an id" : function() {
		...
		var retrievedModel = Data.ModelCache.get( model );
	...
	"get() should *not* return a reference to the first model, when a second one is passed in with the same type (subclass), but not passing in any id's" : function() {
		...
		var retrievedModel1 = Data.ModelCache.get( model1 );
		var retrievedModel2 = Data.ModelCache.get( model2 );
	...
	"get() should return a reference to the first model, when a second one is passed with the same id" : function() {
		...
		var retrievedModel1 = Data.ModelCache.get( model1, 1 );  // same id of
		var retrievedModel2 = Data.ModelCache.get( model2, 1 );  // 1 on both
	...
	"get() should *not* return a reference to the first model, when a second one is passed with the same id, but of a different model type (subclass)" : function() {
		...
		var retrievedModel1 = Data.ModelCache.get( model1, 1 );  // same id of 1 on both,
		var retrievedModel2 = Data.ModelCache.get( model2, 1 );  // but different types of models
	...
	"get() should *not* return a reference to the first model, when a second one is passed with the same type (subclass), but with a different id" : function() {
		...
		var retrievedModel1 = Data.ModelCache.get( model1, 1 );  // same type on both,
		var retrievedModel2 = Data.ModelCache.get( model2, 2 );  // but different id's
...
/*global window, Ext, Y, JsMockito, tests, jQuery, _, Class, Data */
tests.unit.add( new Ext.test.TestSuite( {
	name: 'Data.Model',
	...
	items : [
		{
			...
			"After extending model, the subclass should have a unique __Data_modelTypeId property" : function() {
				...
				Y.Assert.isString( Model.__Data_modelTypeId, "The Model should now have a static __Data_modelTypeId property that is a string" );
			},
			...
			"Attributes should inherit from a Model subclass's superclass, and its superclass as well (i.e. more than one level up)" : function() {
				...
				var Model = Data.Model.extend( {} );
				var SubClassModel = Class.extend( Model, {
					addAttributes : [ 'a', 'b' ]
				} );
			...
			"A superclass that doesn't define any attributes should be skipped for attributes, but the subclass should still inherit from superclasses above it" : function() {
				// Reference the base class, and create two subclasses
				var Model = Data.Model.extend( {} );
				var SubClassModel = Class.extend( Model, {} );  // one that doesn't define any attributes
				var SubSubClassModel = Class.extend( SubClassModel, {
					addAttributes : [ 'a', 'b' ]
				} );
		...
		
		{
			...
			"The getAttributes() static method should retrieve a hashmap of the attributes for the model" : function() {
				var SuperclassModel = Data.Model.extend( {
					attributes : [
						{ name: 'id', type: 'number' },
			...
			items : [
				{
					...
					"Attempting to instantiate a persistenceProxy with no 'type' attribute should throw an error" : function() {
						var TestModel = Class.extend( Data.Model, {
					...
					"Providing a valid config object should instantiate the Proxy *on class's the prototype*" : function() {
						...
						var model = new TestModel();
						Y.Assert.isInstanceOf( Data.persistence.RestProxy, TestModel.prototype.persistenceProxy );
					},
					...
					"Providing a valid config object should instantiate the Proxy *on the correct subclass's prototype*, shadowing superclasses" : function() {
						...
						Y.Assert.isInstanceOf( Data.persistence.RestProxy, TestSubModel.prototype.persistenceProxy );
					}
				},
			...
			
				{
					...
					setUp : function() {
						this.TestModel = Class.extend( Data.Model, {
							addAttributes: [
								{ name: 'attribute1' },
		...
		
		{
			...
			"getIdAttribute() should return the Data.attribute.Attribute referenced by the 'idAttribute' config" : function() {
				var Model = Data.Model.extend( {
					attributes: [ 'id' ],
					idAttribute: 'id'
				} );
				...
				var model = new Model();
				Y.Assert.isInstanceOf( Data.attribute.Attribute, model.getIdAttribute() );
			},
		...
		
		{
			...
			"set() should delegate to the Attribute's beforeSet() and afterSet() methods to do any pre and post processing needed for the value" : function() {
				...
				var TestAttribute = Data.attribute.Attribute.extend( {
		...
		
		{
			...
			"reload() should delegate to its persistenceProxy's read() method to retrieve the data" : function() {
				var proxy = JsMockito.mock( Data.persistence.Proxy.extend( {
					// Implementation of abstract interface
					create : Data.emptyFn,
					read : Data.emptyFn,
					update : Data.emptyFn,
			...
			items : [
				{
					...
					setUp : function() {
						this.proxy = JsMockito.mock( Data.persistence.Proxy.extend( {
							// Implementation of abstract interface
							create: Data.emptyFn,
							read: Data.emptyFn,
							update: Data.emptyFn,
				...
				
				{
					...
					setUp : function() {
						...
						this.collection1 = JsMockito.mock( Data.Collection );
						JsMockito.when( this.collection1 ).getModels().thenReturn( [] );
						
						this.collection2 = JsMockito.mock( Data.Collection );
...
/*global window, jQuery, Ext, Y, JsMockito, Data, tests, _ */
tests.unit.persistence.add( new Ext.test.TestSuite( {
	name: 'Data.persistence.RestProxy',
	...
	items : [
			...
			items : [
				{
					...
					"create() should populate the model with any response data upon a successful ajax request" : function() {
						...
						var TestProxy = Data.persistence.RestProxy.extend( {
				...
				
				{
					...
					"By default, the ajax function should be called with the HTTP method 'POST'" : function() {
						...
						var httpMethod = "";
						var TestProxy = Class.extend( Data.persistence.RestProxy, {
							ajax: function( options ) {
			...
			items : [
				...
				
				{
					...
					setUp : function() {
						...
						this.TestProxy = Data.persistence.RestProxy.extend( {
				...
				
				{
					...
					setUp : function() {						
						this.mockModel = JsMockito.mock( Data.Model );
		...
		
		{
			...
			"buildUrl() should handle a urlRoot without a trailing slash" : function() {
				...
				var proxy = new Data.persistence.RestProxy( {
					urlRoot : '/testUrl',
					appendId : false
				} );
	...
	items : [		
		{
			...
			"Instatiating two models of different types, but the same instance ID, should *not* be 'combined' into the same instance" : function() {
				var ModelClass1 = Data.Model.extend( {
					attributes : [ 'id' ],
					idAttribute : 'id'
				} );
				var ModelClass2 = Data.Model.extend( {
					attributes : [ 'id' ],
					idAttribute : 'id'
				} );
...
/*global window, Ext, Y, JsMockito, tests, Data, _ */
tests.integration.add( new Ext.test.TestSuite( {
...
/*global window, Ext, Y, JsMockito, tests, Class, Data */
tests.integration.add( new Ext.test.TestSuite( {
	...
	items : [
		{
			...
			"A nested Collection attribute should default to be an empty collection" : function() {
				...
				Y.Assert.isInstanceOf( Data.Collection, model.get( 'nestedCollection' ), "The 'nestedCollection' should have been an instance of Data.Collection" );
		...
		
		{
			...
			"set() should only change the attribute that a nested Collection is being set to if it is a different Collection than it already has" : function() {
				...
				var NestedCollection = Data.Collection.extend( {} );
		...
		
		{
			...
			"When an attribute has changed in a deeply nested embedded model/collection, its uppermost parent model should fire 'change' events for every step of the way" : function() {
				...
				var IntermediateModel = Data.Model.extend( {
				...
				var ParentCollection = Data.Collection.extend( {
					model : IntermediateModel,
					toString : function() { return "(ParentCollection)"; }
				} );
				var ChildCollection = Data.Collection.extend( {
					model : ChildModel,
	...
	items : [
		{
			...
			"set() should only change the attribute that a nested Model is being set to if it is a different model than it already has" : function() {
				...
				var NestedModel = Data.Model.extend( {
					attributes : [ 'attr1', 'attr2' ]
				} );
		...
		
		{
			...
			setUp : function() {
				this.ParentWithEmbeddedChildModel = Data.Model.extend( {
					attributes : [
						{ name: 'child', type: 'model', embedded: true }
					]
				} );
				
				this.ParentWithNonEmbeddedChildModel = Data.Model.extend( {
					attributes : [
