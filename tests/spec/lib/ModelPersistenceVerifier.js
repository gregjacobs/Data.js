...
define( [
...
], function( _, Class, Model, LoadOperation, SaveOperation, DestroyOperation ) {
	 ...
	 * {@link #executePersistenceMethod} method should be executed to start the test.
	 * 
	...
	var ModelPersistenceVerifier = Class.create( {
		 ...
		 * on this model, from the {@link #executePersistenceMethod.
		 */
		 ...
		 */
		executePersistenceMethod : function( methodName ) {
			if( !_.contains( [ 'load', 'save', 'destroy' ], methodName ) )
				throw new Error( "`methodName` arg must be one of: 'load', 'save', 'destroy'" );
			
			
			// Block multiple executions of the executePersistenceMethod() method
		...
		verify : function( expectedOutcome ) {
			...
			expect( this.completeEventCount ).toBe( expectedOutcome === 'success' || expectedOutcome === 'error' ? 1 : 0 );
			expect( this.cancelEventCount ).toBe( expectedOutcome === 'cancel' ? 1 : 0 );
