...
define( [
	...
	describe( "unit.attribute.Integer", function() {
		...
		describe( "Test beforeSet()", function() {
			...
			it( "beforeSet() should return the appropriate string value when provided a range of values and types, when the useNull config is false", function() {
				...
				expect( value ).toBe( 1 );  // orig YUI Test err msg: "Test with value: 1.42"
				...
				expect( value ).toBe( 1 );  // orig YUI Test err msg: "Test with value: '1.11'"	
			...
			it( "beforeSet() should strip off $, %, and comma (',') characters from an input string, to make an integer", function() {
				var mockModel = JsMockito.mock( Model ),
				    attribute = new IntegerAttribute( { name: 'attr', useNull: true } ),
				    oldValue,  // undefined
				    value;
				...
				expect( value ).toBe( 1000 );  // orig YUI Test err msg: "Test with value: $1,000.32%"
			} );
